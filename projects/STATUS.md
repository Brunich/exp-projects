...
| api-leads-capture | backend | in_progress | Fastify POST/GET leads, Zod validation, API key auth, JSON persistence, list filters and pagination, CSV export, email deduplication with optional upsert mode, webhook notify with retry queue, dead-letter replay, dead-letter CSV export, dead-letter purge by date, honeypot + rate limiting |
